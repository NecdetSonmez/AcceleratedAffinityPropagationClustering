...
ApcGpuV2::ApcGpuV2(float* points, int pointCount, int pointDimension)
{
	if (pointCount <= 0)
	{
...
void ApcGpuV2::updateResponsibility()
{
    ...
    // Update responsibility with given values
    launchKernel_updateResponsibilityWithMax(blockCount, 32, m_deviceSimilarity, m_deviceResponsibility, m_deviceAvailability, m_pointCount, m_deviceMaxForResponsibility);
}
...
void ApcGpuV2::updateAvailability()
{
    ...
    // Calculate block count
    int blockCount = ((m_pointCount - 1)/ 32) + 1;
	launchKernel_updateAvailability(blockCount, threadCount, m_deviceSimilarity, m_deviceResponsibility, m_deviceAvailability, m_pointCount);
}
